...
int main(int argc, char** argv) {
  try {
    std::string diagnostic_crop;
    std::string diagnostic_dir;
    int diagnostic_repeat = 200;
    for (int i = 1; i < argc; ++i) {
      const std::string arg = argv[i];
      if (arg == "--diagnose-digit-crop" && i + 1 < argc) diagnostic_crop = argv[++i];
      else if (arg == "--diagnose-digit-dir" && i + 1 < argc) diagnostic_dir = argv[++i];
      else if (arg == "--diagnose-repeat" && i + 1 < argc) diagnostic_repeat = std::max(1, std::atoi(argv[++i]));
    }
    for (int i = 1; i < argc; ++i) {
    ...
    if (!diagnostic_crop.empty() || !diagnostic_dir.empty()) {
      std::vector<std::string> image_paths;
      if (!diagnostic_crop.empty()) image_paths.push_back(diagnostic_crop);
      if (!diagnostic_dir.empty()) {
        for (const auto& entry : std::filesystem::directory_iterator(diagnostic_dir)) {
      ...
      if (image_paths.empty()) throw std::runtime_error("digit diagnostic found no input images");

      for (const std::string& image_path : image_paths) {
        cv::Mat diagnostic_image = cv::imread(image_path, cv::IMREAD_COLOR);
        if (diagnostic_image.empty()) throw std::runtime_error("cannot read diagnostic crop: " + image_path);
        const int repeats = image_paths.size() == 1 ? diagnostic_repeat : 1;
        std::vector<float> reference_scores;
        std::map<int, int> class_counts;
        double probability_sum = 0.0;
        double probability_min = 1.0;
        double probability_max = 0.0;
        double max_output_delta = 0.0;
        double inference_ms_sum = 0.0;

        for (int run = 0; run < repeats; ++run) {
          digit_pre.run(diagnostic_image, digit_input);
          inference_ms_sum += digit_cls.run(cls_stream, digit_input);
          std::vector<float> scores = digit_cls.output_as_float(0);
          const ClassResult result = classify_from_output(scores, digit_labels);
          ++class_counts[result.id];
          probability_sum += result.prob;
          probability_min = std::min(probability_min, static_cast<double>(result.prob));
          probability_max = std::max(probability_max, static_cast<double>(result.prob));
          if (reference_scores.empty()) {
            reference_scores = scores;
          } else if (reference_scores.size() == scores.size()) {
            for (size_t j = 0; j < scores.size(); ++j) {
              max_output_delta = std::max(max_output_delta,
                  std::abs(static_cast<double>(scores[j]) - static_cast<double>(reference_scores[j])));
            }
          }
        }

        std::vector<size_t> order(reference_scores.size());
        std::iota(order.begin(), order.end(), 0);
        std::partial_sort(order.begin(), order.begin() + std::min<size_t>(5, order.size()), order.end(),
            [&](size_t a, size_t b) { return reference_scores[a] > reference_scores[b]; });
        const double score_sum = std::accumulate(reference_scores.begin(), reference_scores.end(), 0.0);
        std::cout << "{\"diagnostic\":\"digit_crop\",\"file\":\"" << json_escape(image_path)
                  << "\",\"repeat\":" << repeats << ",\"class_counts\":{";
        bool first_count = true;
        for (const auto& [id, count] : class_counts) {
          if (!first_count) std::cout << ",";
          std::cout << "\"" << id << "\":" << count;
          first_count = false;
        }
        std::cout << "},\"prob_mean\":" << probability_sum / repeats
                  << ",\"prob_min\":" << probability_min
                  << ",\"prob_max\":" << probability_max
                  << ",\"score_sum\":" << score_sum
                  << ",\"max_output_delta\":" << max_output_delta
                  << ",\"mean_inference_ms\":" << inference_ms_sum / repeats
                  << ",\"top5\":[";
        const size_t top_count = std::min<size_t>(5, order.size());
        for (size_t rank = 0; rank < top_count; ++rank) {
          if (rank > 0) std::cout << ",";
          const size_t id = order[rank];
          std::cout << "[" << id << "," << reference_scores[id] << "]";
        }
        std::cout << "]}" << std::endl;
      }
