...
struct PoseParseStats {
  ...
  bool selection_changed_from_legacy{false};
};
...
static std::vector<Detection> parse_pose_detections(const std::vector<float>& out, const LetterboxMeta& meta, float conf,
  ...
  std::vector<Detection> legacy_selected;
  for (const auto& d : candidates) {
    const bool duplicate = std::any_of(legacy_selected.begin(), legacy_selected.end(), [&](const Detection& kept) {
      return rect_iou(d.roi, kept.roi) > nms_iou;
    });
    if (duplicate) continue;
    legacy_selected.push_back(d);
    if (max_detections > 0 && static_cast<int>(legacy_selected.size()) >= max_detections) break;
  }
  local_stats.legacy_selected = legacy_selected.size();
  ...
  if (selected.size() != legacy_selected.size()) {
    local_stats.selection_changed_from_legacy = true;
  } else {
    for (const auto& d : selected) {
      const bool found = std::any_of(legacy_selected.begin(), legacy_selected.end(), [&](const Detection& legacy) {
        return d.roi == legacy.roi && std::abs(d.score - legacy.score) < 1e-6f;
      });
      if (!found) {
        local_stats.selection_changed_from_legacy = true;
        break;
    ...
    while (true) {
      ...
      if (cfg.print_every > 0 && frames_total % cfg.print_every == 0) {
        ...
        std::cout << "{"
                  ...
                  << "\"legacy_det_count\":" << pose_stats.legacy_selected << ","
                  << "\"selection_changed\":" << (pose_stats.selection_changed_from_legacy ? "true" : "false") << ","
