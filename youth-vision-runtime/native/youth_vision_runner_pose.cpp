...
static void set_config_value(Config& c, const std::string& key, const std::string& value) {
  ...
  else if (k == "det_size") c.det_size = std::atoi(v.c_str());
...
class LetterboxPreprocessor {
 public:
  explicit LetterboxPreprocessor(int target) : target_(target) {
  ...
  LetterboxMeta run(const cv::Mat& src, float* dst) {
    ...
    m.scale = std::min(static_cast<double>(target_) / src.cols, static_cast<double>(target_) / src.rows);
...
static std::vector<Detection> parse_pose_detections(const std::vector<float>& out, const LetterboxMeta& meta, float conf,
  ...
  for (size_t i = 0; i + kPoseRow <= out.size(); i += kPoseRow) {
    float score = out[i + 4];
    if (score < conf || out[i + 2] <= out[i] + 2.0f || out[i + 3] <= out[i + 1] + 2.0f) continue;
  ...
  for (const auto& d : candidates) {
    ...
    for (const auto& kept : selected) {
      if (rect_iou(d.roi, kept.roi) > nms_iou) {
        duplicate = true;
        break;
      }
    }
...
static ClassResult classify_from_logits(const std::vector<float>& logits, const std::vector<std::string>& labels) {
  ClassResult r;
  if (logits.empty()) return r;
  r.id = static_cast<int>(std::max_element(logits.begin(), logits.end()) - logits.begin());
  float maxv = *std::max_element(logits.begin(), logits.end());
  double sum = 0.0;
  for (float v : logits) sum += std::exp(static_cast<double>(v - maxv));
  r.prob = sum > 0.0 ? static_cast<float>(std::exp(static_cast<double>(logits[r.id] - maxv)) / sum) : 0.0f;
...
int main(int argc, char** argv) {
  try {
    ...
    if (det.input_w() != cfg.det_size || det.input_h() != cfg.det_size) {
      std::cerr << "warning: det engine input is " << det.input_w() << "x" << det.input_h()
                << ", config det_size=" << cfg.det_size << "\n";
      cfg.det_size = det.input_w();
    }
    ...
    LetterboxPreprocessor det_pre(cfg.det_size);
    ...
    while (true) {
      ...
      std::vector<Detection> detections = parse_pose_detections(
          ...
          cfg.max_detections, cfg.nms_iou);
      ...
      for (size_t det_idx = 0; det_idx < detections.size(); ++det_idx) {
        ...
        if (extract_directional_square(frame, d, cfg.canonical_crop_size, canonical_crop)) {
          if (crop_preview.empty()) crop_preview = canonical_crop.clone();
          if (mode == "digit") {
            digit_pre.run(canonical_crop, digit_input);
            cls_ms += digit_cls.run(cls_stream, digit_input);
            class_results[det_idx] = classify_from_logits(digit_cls.output_as_float(0), digit_labels);
          } else {
            image_pre.run(canonical_crop, image_input);
            cls_ms += image_cls.run(cls_stream, image_input);
            class_results[det_idx] = classify_from_logits(image_cls.output_as_float(0), image_labels);
          }
        }
      }
      ...
      if (cfg.overlay_draw || !cfg.overlay_jpeg.empty() || !cfg.crop_jpeg.empty()) {
        ...
        if (cfg.overlay_draw || !cfg.overlay_jpeg.empty()) {
          draw_overlay(frame, detections, class_results, mode, fps, pre_avg, det_avg, cls_avg);
        }
        if (cfg.overlay_every > 0 && frames_total % cfg.overlay_every == 0) {
          if (!cfg.overlay_jpeg.empty()) write_jpeg_atomic(cfg.overlay_jpeg, frame);
          if (!cfg.crop_jpeg.empty()) {
            ...
            write_jpeg_atomic(cfg.crop_jpeg, crop_preview);
          }
        }
      }
