...
struct Config {
  ...
  double record_duration_sec{300.0};
};
...
static Config parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--config") {
      ...
      std::cout << "usage: " << argv[0]
                ...
                << " [--record-file output.mp4 --record-duration-sec 300]\n";
      std::exit(0);
    }
  }
  ...
  if (!c.record_file.empty()) {
    ...
    if (c.record_duration_sec <= 0.0 || c.record_duration_sec > 300.0) {
      throw std::runtime_error("record_duration_sec must be in (0, 300]");
    }
  }
  return c;
}
...
class AsyncMvsCapture {
 public:
  AsyncMvsCapture(int index, float fps, float exposure_us, float gain,
                  AsyncVideoRecorder* recorder = nullptr)
      : cap_(index, fps, exposure_us, gain), recorder_(recorder), worker_([this] { loop(); }) {}
    ...
    if (use_mvs) {
      if (cfg.async_capture) {
        async_mvs_cap.reset(new AsyncMvsCapture(
            cfg.mvs_index, cfg.mvs_fps, cfg.mvs_exposure_us, cfg.mvs_gain, recorder.get()));
      } else {
    ...
    while (!g_stop_requested.load()) {
      ...
      for (size_t det_idx = 0; det_idx < detections.size(); ++det_idx) {
        ...
        if (extract_directional_square(frame, d, cfg.canonical_crop_size, canonical_crop)) {
          if (emit_jpeg) crop_snapshots.emplace_back(det_idx, canonical_crop.clone());
          if (mode == "digit") {
      ...
      if (cfg.overlay_draw || (emit_jpeg && (!cfg.overlay_jpeg.empty() || !cfg.crop_jpeg.empty() || !cfg.crops_dir.empty()))) {
        ...
        if (emit_jpeg) {
          ...
          if (!cfg.crops_dir.empty()) {
            ...
            for (size_t rank = 0; rank < crop_snapshots.size(); ++rank) {
              ...
              cv::Point2f center{};
              for (const cv::Point2f& point : d.class_quad) center += point;
              center *= 0.25f;
      ...
      if (has_det || periodic_status) {
        ...
        for (size_t j = 0; j < detections.size(); ++j) {
          ...
          cv::Point2f item_center{};
          for (const cv::Point2f& point : item.class_quad) item_center += point;
          item_center *= 0.25f;
          if (j > 0) record << ",";
          record << "{"
                 << "\"detection_index\":" << j << ","
