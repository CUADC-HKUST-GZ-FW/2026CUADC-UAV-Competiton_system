...
class FcuInterfaceMavrosNode(Node):
    ...
    def select_composite_start_seq(
        ...
        item seq1 before calling WaypointSetCurrent.  If the aircraft is already
        inside the replaced original segment, start at A.  If it is already past
        the resume point, map to the matching original waypoint in the composite
        suffix instead of jumping backwards.
        """
        ...
        elif current_seq < resume_index:
            selected_seq = a_seq
            reason = 'current_inside_replaced_segment_start_at_A'
        else:
            suffix_start_seq = d_seq + 1
            selected_seq = suffix_start_seq + (current_seq - resume_index)
            reason = 'map_original_suffix_current_seq'
