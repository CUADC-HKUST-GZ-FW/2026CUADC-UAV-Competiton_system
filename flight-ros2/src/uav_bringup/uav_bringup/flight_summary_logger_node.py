...
class FlightSummaryLoggerNode(Node):
    ...
    def waypoints_callback(self, msg):
        ...
        release_indices = [
            ...
            if command == self.MAV_CMD_DO_SET_SERVO
        ]
        if self.release_seq is None:
            self.release_seq = release_indices[0] if release_indices else None
        if self.r_seq is None:
            self.r_seq = (
                self.release_seq - 1
                if self.release_seq is not None and self.release_seq > 0
                else None
            )
        if self.d_seq is None:
            self.d_seq = (
                ...
                and self.release_seq + 1 < len(msg.waypoints)
                else None
            )
