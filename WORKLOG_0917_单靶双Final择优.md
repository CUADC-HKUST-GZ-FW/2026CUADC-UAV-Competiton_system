...
5. 如果第一个 final 后 `10 s` 仍没有第二个，则发布当前唯一最佳包，避免无限等待；
...
本轮未执行真实飞行。兜底时间已按最新决定从 `25 s` 缩短为 `10 s`。当天两个已知
任务的爬升包到平飞包间隔约为 20 至 21 秒，因此 10 秒兜底可能在第二个平飞 final
到来前提交首包。下一次实飞需要重点确认首包质量、第二包出现时间以及航点 5 前的
航线改写结果；若仍出现爬升包抢先，应改用航点触发选择或重新放宽等待时间。
