...
acquire_singleton_lock() {
    ...
    if ! flock --nonblock "${LOCK_FD}"; then
        ...
        echo "competition-target fusion is already running: ${owner_description}" >&2
        exec {LOCK_FD}>&-
        LOCK_FD=""
        return 73
    fi
...
competition selector|/scripts/competition_selector.py
...
for required in \
    ...
    "${ROOT}/scripts/competition_selector.py" \
    "${ROOT}/scripts/competition_selector_core.py" \
...
if [[ "$(node_count_in_snapshot "${startup_node_snapshot}" /competition_selector)" != "0" ]]; then
    fail "ROS graph already contains /competition_selector"
fi
...
setsid bash -lc \
    "source '${ROS_SETUP}'; source '${WS_SETUP}'; exec ros2 run uav_vision_bridge vision_target_bridge_node --ros-args -p source_topic:='/vision/competition_selected_target' -p heading_deg:='${HEADING_DEG}' -p auto_execute:=true" \
    >"${BRIDGE_LOG}" 2>&1 </dev/null {LOCK_FD}>&- &
bridge_pid=$!
register_child "${bridge_pid}" "vision_target_bridge" "${RUN_DIR}/competition_target_fusion_bridge.pid"
...
while true; do
    ...
    selector_node_count="$(
        node_count_in_snapshot "${graph_node_snapshot}" /competition_selector
    )"
    mission_manager_count="$(
    ...
    if ((recon_node_count > 1 || selector_node_count > 1 || bridge_node_count > 1 \
    ...
    selected_subscribers="$(awk '/Subscription count:/ {print $3; exit}' <<<"${selected_info}")"
    ...
    selected_publishers="${selected_publishers:-0}"
    selected_subscribers="${selected_subscribers:-0}"
    ...
    if ((recon_publishers > 1 || selected_publishers > 1 || target_publishers > 1)); then
...
echo "selected_result=${RESULT_DIR}/competition_selected.json"
echo "selected_topic=/vision/competition_selected_target"
echo "flight_target_topic=/vision/target_command"
if [[ -n "${YOUTH_RECORD_FILE:-}" ]]; then
...
if [[ "${UAV_DETACHED:-0}" == "1" ]]; then
    echo "stop_command=${UAV_ENTRY_SCRIPT_DIR}/stop_detached_uav.sh competition"
else
    echo "press Ctrl+C to stop all processes started by this command"
fi
echo "warning=Ctrl+C stops companion processes but does not guarantee cancellation of an FCU mission already uploaded"
