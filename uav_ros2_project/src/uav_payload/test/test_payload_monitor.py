...
class PayloadMonitorTest(unittest.TestCase):
    def make_monitor(self, **overrides):
        ...
        monitor.start_mission_monitoring(0.0)
        return monitor
    ...
    def test_inactive_task_does_not_monitor_retained_fcu_mission(self):
        monitor = PayloadMonitor(PayloadMonitorConfig())
        monitor.set_task_id('none', 0.0)

        events = monitor.observe_mission(mission(release_seq=4), 0, 1.0)

        self.assertEqual([], events)
        self.assertEqual(PayloadMonitorState.IDLE, monitor.state)
        self.assertFalse(monitor.release_command_seen)

    def test_unconfirmed_task_end_returns_monitor_to_idle(self):
        monitor = self.make_monitor()
        monitor.observe_mission(mission(release_seq=4), 0, 1.0)
    ...
    def test_active_task_waits_for_verified_upload_before_scanning(self):
        monitor = PayloadMonitor(PayloadMonitorConfig())
        monitor.set_task_id('target_001', 0.0)

        before_upload = monitor.observe_mission(mission(release_seq=4), 0, 1.0)
        monitor.start_mission_monitoring(2.0)
        after_upload = monitor.observe_mission(mission(release_seq=4), 0, 2.1)

        self.assertEqual([], before_upload)
        self.assertEqual(['command_uploaded'], [event.key for event in after_upload])
